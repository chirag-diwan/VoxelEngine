...
void Texture::LinkCubeMap(const char* const filePaths[], size_t size) {
    Bind();  // Ensure bound (extra safety)

    int width = 0, height = 0, nrChannels = 0;
    unsigned char* data = nullptr;
    bool allLoaded = true;  // Track completeness

    for (size_t i = 0; i < size; ++i) {
        data = stbi_load(filePaths[i], &width, &height, &nrChannels, 0);
        if (data != nullptr) {
            GLenum internalFormat = (nrChannels == 4) ? GL_RGBA : GL_RGB;  // Fix: Dynamic format
            GLenum format = (nrChannels == 4) ? GL_RGBA : GL_RGB;
            glTexImage2D(
                GL_TEXTURE_CUBE_MAP_POSITIVE_X + static_cast<GLint>(i),  // Cast for safety
    ...
    warpAndFilter();  // Apply params
}

void Texture::warpAndFilter() {
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    // Optional: Add mips if needed later
